...
function largestP(a) {
  ...
  const func2 = (c) => {
    // for filtering symbols from words
    const len = c.length;
    let letchar;
  ...
  const func1 = (c) => {
    // for filtering palindrome like words
    const len = c.length;
    return c[0] === c[len - 1];
  };
  const func3 = (c) => {
    // for filtering actual palindrome words
    const len = c.length;
...
console.log(
  largestP("Hello Mam, how are you. Madam, do you know racecar?")
);